...
double CalculateExpression(CString strExpr) {
	...
	for (int i = 0; i < len; i++) {		//문자열의 길이만큼 반복
		...
		if (!ops.empty()) ops.pop(); // '(' 제거
			...
			if ((c == _T('+') || c == _T('-')) && (i == 0 || strExpr[i - 1] == _T('('))) {
				values.push(0.0);
			}
...
void CMFCApplication2Dlg::AppendString(CString strInput)	//======버튼 선택시 문자열 처리하는 함수=======
{
	...
	if (bIsInputOp) {
		if (lastChar == _T('(') && (strInput == _T("*") || strInput == _T("/"))) {	//'('뒤에 바로 곱하기와 나누기를 쓸 수 없음
			m_strInfo = _T("괄호 뒤에는 '*'또는 '/'가 올 수 없습니다.");
			UpdateData(FALSE);
			return;
		}
	}
...
void CMFCApplication2Dlg::OnBnClickedSquare()	//======'x^2'버튼 클릭======
{
	...
	double dResult1 = CalculateExpression(m_strResult);
	m_strResult.Format(_T("%g"), dResult1);

	m_bIsCalculated = TRUE;

	CString strHistory;
	strHistory.Format(_T("%s = %s"), m_strSaveResult, m_strResult);
...
void CMFCApplication2Dlg::OnBnClickedResult()	//======'='버튼 클릭======
{
	...
	if (m_strLast == _T('+') || m_strLast == _T('-') || m_strLast == _T('*') || m_strLast == _T('/')) {
		m_strInfo = _T("마지막에 연산자가 올 수 없습니다.");
		return;
	}
